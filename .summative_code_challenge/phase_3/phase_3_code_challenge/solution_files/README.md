...
You have been asked by a manufacturer to build a machine learning model to help with quality assurance.  Some fraction of all parts created in their factory have manufacturing flaws, and therefore should not be shipped to customers.  You have been asked to develop a "best overall model". Therefore they would like a **classification model** that optimizes **f1 score**. In meetings with the manufacturer, some of the stakeholders expressed concern about the cost of processing a returned part (false negative) so in addition to reporting the f1 score they ask that you report the **recall score** for your final model.
...
Check the performance of the logistic regression model on the training data, using the sklearn `f1_score` function ([documentation here](https://scikit-learn.org/stable/modules/generated/sklearn.metrics.f1_score.html)).  You can just compute the score, or a level-up would be to use `f1_score` within `cross_val_score` ([documentation here](https://scikit-learn.org/stable/modules/generated/sklearn.model_selection.cross_val_score.html)).  **Note:** both y-values should come from the training data; you are not using the test data yet.
...
Evaluate the final model based on the test (holdout) data.  Report how it performs in terms of `f1_score`, and interpret this for a business audience.

Evaluate the `recall_score` for the final model, and interpret this for a business audience.
...
Before you submit your modeling analysis, make sure you have completed all of the steps:
 ...
 - [ ] Fit a `LogisticRegression` model on `X_train` and investigated its performance in terms of `f1_score`
 ...
 - [ ] Evaluated the final model's performance on `X_test` using the requested metrics, and interpreted for a business audience
...
Is the `f1_score` meaningfully different?  Would your answer about dropping certain features change?
